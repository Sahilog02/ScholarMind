...
Pydantic (structured outputs) · OpenRouter (`openrouter/free`, free tier,
default) — or Gemini / Groq / OpenAI · Streamlit
...
- Get a free OpenRouter key at https://openrouter.ai/keys — no credit card
  required (default provider — `OPENROUTER_MODEL` defaults to
  `openrouter/free`, OpenRouter's free-model router, which automatically
  picks an available free, open-source model for each request. To pin a
  specific model instead, browse https://openrouter.ai/models?max_price=0
  and use its slug, e.g. `meta-llama/llama-3.3-70b-instruct:free`).
- Alternatively set `LLM_PROVIDER=gemini` and get a free key at
  https://aistudio.google.com/apikey — no credit card required.
  `GEMINI_MODEL` defaults to `gemini-2.5-flash` (Gemini's Pro models are
  paid-only; `gemini-2.5-flash-lite` is a free alternative with a higher
  daily request limit if you hit 429 rate-limit errors on flash).
- Alternatively set `LLM_PROVIDER=groq` and get a free Groq key at
  https://console.groq.com/keys (`GROQ_MODEL` defaults to
  `openai/gpt-oss-120b`; Groq deprecates model names fairly often, check
  https://console.groq.com/docs/models if it errors).
...
**Batch evaluation** (runs all 100 claims in `eval/claims.json`, auto-approves
...
│   ├── config.py                   LLM provider switch (OpenRouter/Groq/OpenAI)
...
│       └── synthesizer.py
├── eval/
│   ├── claims.json                 100 test claims spanning ML/AI topics
